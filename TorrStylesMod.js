(function () {
  ...
  var TH = {
    seeds: { danger_below: 5, good_from: 10, top_from: 20 },
    bitrate: { warn_from: 50, orange_from: 75, danger_from: 100 },
    size: { mid_from_gb: 50, high_from_gb: 100, top_from_gb: 200 },
    peers: { high_from: 10 },
    debounce_ms: 60
  };
  ...
  function tsExpandSelectors(selector) {
    return FOCUS_SCOPES.map(function (prefix) { return prefix + selector; }).join(', ');
  }
  ...
  function buildStyles() {
    ...
    // Size
    tsSetRule(styles, '.torrent-item__size.ts-size', Object.assign(appleBadge(APPLE.green, { bg: 0.12 }), { 'font-weight': '700' }));
      ...
      var css = Object.keys(styles)
        .map(function (selector) {
          ...
          var rules = Object.keys(props)
            .map(function (prop) { return prop + ': ' + props[prop] + ' !important'; })
            .join('; ');
  ...
  function scheduleUpdate(delayMs) {
    try { if (tsUpdateTimer) clearTimeout(tsUpdateTimer); } catch (e) {}
  ...
  function init() {
    ...
    console.log(config.name, 'loaded, version:', config.version, '(no host restrictions)');
  }
